...
from ._base import USER_AGENT, _build_payload, _decompress_result, _now, _raise_api_error
from ._errors import Browser7Error, RenderError
from ._types import AccountBalance, RegionsResponse, RenderResult


class AsyncBrowser7:
    """
    Browser7 API client (asynchronous).
    ...
    Example:
        >>> client = AsyncBrowser7(api_key='b7_your_api_key')
        >>> result = await client.render('https://example.com')
        >>> print(result.html)

    Async context manager:
        >>> async with AsyncBrowser7(api_key='b7_your_api_key') as client:
        ...     result = await client.render('https://example.com')
    """
    ...
    def __init__(
        ...
        if not api_key:
            raise Browser7Error("API key is required")
    ...
    async def __aenter__(self) -> "AsyncBrowser7":
        return self
    ...
    async def _emit(self, on_progress: Optional[Callable], event: Dict[str, Any]) -> None:
        """Emit a progress event, supporting both sync and async callbacks."""
    ...
    async def render(
        ...
        *,
        ...
        Args:
            ...
            on_progress: Optional sync or async callback for progress events
        ...
        response = await self.create_render(
            url,
        ...
        await self._emit(on_progress, {'type': 'started', 'renderId': render_id, 'timestamp': _now()})

        await asyncio.sleep(2)

        for attempt in range(max_attempts):
            result = await self.get_render(render_id)
            ...
            if result.status == 'completed':
                await self._emit(on_progress, {'type': 'completed', 'renderId': render_id, 'timestamp': _now(), 'status': result.status})
                return result

            if result.status == 'failed':
                await self._emit(on_progress, {'type': 'failed', 'renderId': render_id, 'timestamp': _now(), 'status': result.status})
                raise RenderError(
                    f"Render failed: {result.error or 'Unknown error'}",
                    None, {'status': 'failed', 'error': result.error},
                    getattr(result, 'error_code', None), render_id, getattr(result, 'billable', None),
                )

            await asyncio.sleep(result.retry_after)

        raise RenderError(
            f"Render timed out after {max_attempts} attempts",
            None, None, 'RENDER_TIMEOUT', render_id, None,
        )
    ...
    async def create_render(
        ...
        *,
        ...
        try:
            ...
            raise Browser7Error(f"Failed to connect to {renders_url}: {e}")

        if not response.is_success:
            _raise_api_error(response.status_code, response.text, 'Failed to start render')

        return response.json()

    async def get_render(self, render_id: str) -> RenderResult:
        """
        ...
        try:
            ...
            raise Browser7Error(f"Failed to connect to {status_url}: {e}")

        if not response.is_success:
            _raise_api_error(response.status_code, response.text, 'Failed to get render status')
    ...
    async def get_account_balance(self) -> AccountBalance:
        ...
        try:
            response = await self._client.get(balance_url)
        except httpx.RequestError as e:
            raise Browser7Error(f"Failed to connect to {balance_url}: {e}")

        if not response.is_success:
            _raise_api_error(response.status_code, response.text, 'Failed to get account balance')
    ...
    async def get_regions(self) -> RegionsResponse:
        ...
        try:
            response = await self._client.get(regions_url)
        except httpx.RequestError as e:
            raise Browser7Error(f"Failed to connect to {regions_url}: {e}")

        if not response.is_success:
            _raise_api_error(response.status_code, response.text, 'Failed to get regions')
